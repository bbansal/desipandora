package com.desipandora.server;
...
public class DesiPandoraServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (checkValidOperatonCode(operationCode, reqString, resp)) {
			logger.info("handling request:" + reqString);
	...
	private void handleGetNextSongs(HttpServletRequest req,
		...
		if (null == sessionId)
			throw new RuntimeException("empty sessionId passed in search.");

		List<SongEntry> songs = _service.getNextSongs(sessionId, numSongs);
		logger.debug("request Nextsong  for session_id" + sessionId + " songs:" + songs);
		
		resp.getWriter().print(GetJsonSongList(songs).toString());
	}

	private JSONObject GetJsonSongList(List<SongEntry> songs) throws JSONException {
	...
	private void handleSearch(HttpServletRequest req, HttpServletResponse resp)
		...
		// create new sessionId for each search.
